    ...
    claude)
      ...
      if is_enabled "${AI_REVIEW_CLAUDE_USE_BUILTIN_REVIEW:-true}" \
        ...
        "${agent_env[@]}" "$claude_cmd" -p "$prompt" \
          ${claude_flags[@]+"${claude_flags[@]}"} \
          --append-system-prompt "After completing the review, your FIRST line of output MUST be exactly STATUS: PASS or STATUS: FAIL (bare plain text only — no markdown bold, no asterisks, no backticks around it). CRITICAL OUTPUT FORMAT: To stay within the token budget, use compact single-line format for EVERY scorecard entry. For System Quality Attributes use: **AttributeName**: one-sentence evidence. Source: file/test. 5/5 PASS. Improvement: none. For FR/NFR/AC use bullet format with three lines: - Evidence: ...; - Score: 5/5 PASS; - Source: file. For NFR catalog and expanded quality use the same three-line bullet format. Do NOT write multi-paragraph descriptions per attribute — every attribute must fit on one or two lines. Complete ALL required attributes in compact format rather than declaring token exhaustion. MANDATORY SECTION REQUIREMENTS: (1) 'Automated Test And CI Coverage' section MUST explicitly state that ALL/every FR/NFR has repeatable automated coverage — use words like 'all requirements', 'every FR', 'each NFR', 'all tests are repeatable', 'every acceptance criterion has automated coverage'; include coverage/gap assessment words ('covered', 'coverage', 'verified', or 'gap'/'missing'); reference GitHub CI workflow checks ('github', 'workflow', 'pipeline', 'check', 'required'). (2) 'Test Case Matrix' section MUST include: positive/happy-path cases, negative/failure/error cases, edge/boundary cases; map each case to FR/NFR IDs; state 'every' or 'all' FRs/NFRs are covered by tests; explicitly state 'none' or 'missing' for any identified gaps; mention test types (automated/unit/integration/behat/phpunit/manual)." \
          --output-format text \
          >"$output_file" 2>"${output_file}.log"
      fi
      ;;
