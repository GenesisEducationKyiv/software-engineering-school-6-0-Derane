...
GATE_DEFINITION_VALIDATION_SUPPORT_FILE_PATTERN='^(specs/[^/]+/.*\.md|var/manual-test-evidence/.*\.md)$'
...
**Quick verification**: Run `make lint && make psalm && make deptrac && make test`
...
3. Verify: `make lint && make psalm && make deptrac && make test`
4. Commit with reference
...
| Code style              | Run `make lint` (autofix `vendor/bin/phpcbf`) |
...
- **testing-workflow**: Test coverage (PHPUnit Unit + Integration suites)
