 ...
 * so the bounded-retry mechanics are exercised against a mocked AMQPChannel. The
 * retry/park republish runs on a DEDICATED channel obtained via
 * consumeChannel->getConnection()->channel(); the original is acked on the
 * consume channel only after the broker confirms the copy.
 *
 * (The Sending-side ack/nack/DLQ DECISIONS are tested against the MessageConsumer
 * port in SendReleaseEmailConsumerTest.)
 */
final class RabbitConsumerTest extends TestCase
{
    private const RETRY_QUEUE = 'notifications.send-email.retry';
    private const VALID_JSON = '{"schema":"SendReleaseEmail/v1"}';
    ...
    public function testRequeueWithoutRetryIncrementParksForTheGivenDelayPreservingTheRetryCount(): void
    {
        ...
        $acked = [];
        $channel = $this->channel($acked, $this->publishChannel($published));
        $message = $this->message($channel, deliveryTag: 8, retryCount: SendReleaseEmailConsumer::MAX_REDELIVERIES - 1);
        ...
        $headers = $copy->get('application_headers');
        self::assertInstanceOf(AMQPTable::class, $headers);
        self::assertSame(SendReleaseEmailConsumer::MAX_REDELIVERIES - 1, $headers->getNativeData()['x-retry-count']);
    ...
    {
        return new RabbitConsumer(new RabbitConnection($channel));
    }
